<?php
    ...
    if($conn->connect_error){
        ...
        if(!$like){
            ...
            removeComment($conn, $post_id, $user_id, $com);
            $response["resp"] = "Comment removed";
        }
    ...
    function removeComment($conn, $post_id, $user_id, $com){
        $stmt = $conn->prepare("DELETE FROM comments WHERE post_id = ? and user_id = ? and com = ?");
        if($conn->connect_error){
            die("Connection Failed: ".$conn->connect_error);
        }else{
            $stmt->bind_param("iis", $post_id, $user_id, $com);
            $stmt->execute();
        }
    }
?>